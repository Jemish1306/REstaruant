...
import PercalPaymentBillModal from './../../Model/PercalPaymentBillModel';
...
const ParcelOrderManagement = () => {
    ...
    const fetchOrders = async () => {
        try {
            const response = await axios.get('http://localhost:3229/api/percel');
    ...
    const updateOrderStatus = async (id, status) => {
        try {
            const response = await axios.put(`http://localhost:3229/api/percel/${id}`, { status });
            const updatedOrder = response.data;
            setAllOrders(allOrders.map(order => order._id === id ? updatedOrder : order));
        } catch (error) {
    ...
    const renderOrdersTable = (status) => (
        <table className="min-w-full mb-8 border-separate border-spacing-y-2 bg-secondary ">
            <thead>
                <tr className=" bg-secondary text-center gap-4">
                    ...
                    <th className="py-2 border-b px-4   bg-gradient-to-r from-yellow-500 to-blue-500 shadow-lg rounded-md text-black">Quantity</th>
                    ...
                    {/* <th className='py-2 botder-b px-4 text-white'>PaymentType</th> */}
            ...
            <tbody>
                {allOrders.filter(order => order.status === status).map(order => (
                    <tr key={order._id} className="hover:bg-accent rounded-lg  items-center text-center">
                        ...
                        {status === 'Request For Payment' && (
                            <td className="p-2 border-b flex items-center justify-center">
                                <button
                                    className="text-green-500 hover:text-green-700 mr-2 shadow-lg rounded-md bg-accent w-8 h-8 flex items-center justify-center"
            ...
            <div className="container mx-auto p-4 bg-secondary">
                <div className="flex space-x-4 mb-4">
                    {['Request For Payment', 'In Progress', 'Delivered'].map(tab => (
                        <button
                            key={tab}
                            className={`px-4 py-2 rounded ${activeTab === tab ? 'bg-[#FFB238] text-black text-xl font-semibold' : 'bg-accent text-white'}`}
